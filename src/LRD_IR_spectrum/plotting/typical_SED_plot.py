...
def plot_observed_table(ax: Axes, 
                     *,
                     x: str, 
                     y: str, 
                     table: QTable):
    ...
    scatter_handles = {}  # 收集 scatter 的 handles
    ...
    for is_detected in [True, False]:  # 分别绘制 detection 和 non-detection 的数据
        ...
        # 使用不同的 marker 属性
        if is_detected:
            marker_props = dict(marker='o', color='r', edgecolor='k', lw=1)
        else:
            marker_props = dict(marker='v', color='none', edgecolor='r', lw=1.5)
    ...
    observed_sed_line, = ax.plot(x, y, data=table[mask_sed_line],
            ...
            label='Observed SED (A2744-45924)'
            )
...
class TypicalSEDPlot:
    ...
    def plot_panel_A(
        ...
        *,
        ...
        for A_V_val in A_V_list:
            model = self.model_factory(A_V=A_V_val, n_0=n_0, gamma=gamma)  # 固定 (n_0, gamma)
            color = cmap(norm(A_V_val))
            
            _line, = ax.loglog(model.opacity.wavelength, model.calc_nu_L_nu(), color=color, label=rf'{A_V_val:.1f} mag')  # IR re-emission
            handles_ax0_right.append(_line)
            
            ax.plot(model.incident_SED.wavelength, model.incident_SED.nu_L_nu, color=color, **props_incident)  # de-redden 入射光谱
        ...
        _handle_reprocessed_sed = Line2D([], [], color='k', **props_reprocessed, label='Reprocessed SED') 
        ...
        ax.legend(handles=handles_ax0_right[::-1], # 反序，这样图例的上下顺序与 SED 曲线一致
    ...
    def plot_panel_B(
        ...
        *, 
        ...
        # 图例
        ax.legend(handles=handles_ax1_right, loc='upper left', bbox_to_anchor=(0.02, bbox_to_anchor_y), title=r"$A_V$ (upper limit)", )
    ...
    def plot_figure(
        ...
        *, 
        ...
        self.plot_panel_A(ax=axs[0], A_V_list=np.arange(.2, 1.+1e-8, .2), **panel_A_paras)
